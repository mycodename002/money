...
$sql = "SELECT id, title, details FROM `group_mem` WHERE is_deleted = 1 LIMIT $start,4;";
...
<div class="container mx-auto px-4 mt-4 shadow-md ">
    ...
    <div class="overflow-x-auto mt-8">
        ...
        <div class="join mt-4">
            ...
            if ($page > 3) { ?>
                <button class="join-item btn" onclick="window.location.href = '<?php echo base_url('VIEW/SUPPORT/showGroup.php?page=1'); ?>'">1</button>
                <?php if ($page > 4) { ?>
            ...
            for ($i = $start_page; $i <= $end_page; $i++) { 
                ...
                ?>
                <button class="join-item btn <?php echo $active_class; ?>" onclick="window.location.href = '<?php echo base_url('VIEW/SUPPORT/showGroup.php?page='.$i); ?>'">
                    <?php echo $i; ?>
                </button>
            <?php }
            ...
            if ($page < $count_btn - $range) { ?>
                <?php if ($page < $count_btn - $range - 1) { ?>
                    <button class="join-item btn">...</button>
                <?php } ?>
                <button class="join-item btn" onclick="window.location.href = '<?php echo base_url('VIEW/SUPPORT/showGroup.php?page='.$count_btn); ?>'">
                    <?php echo $count_btn; ?>
                </button>
            <?php } ?>
        </div>
...
<dialog id="Modal_restore" class="modal">
    <div class="modal-box">
        <h3 class="font-bold text-lg mb-4">กู้คืนกลุ่ม</h3>
        
        <form id="restoreForm" action="../../API/support/process.php" method="POST">
            <input type="hidden" id="restore_id" name="id">
                <input type="hidden" id="" name="action" value="restoreGroup">

            <div class="modal-action">
                <!-- ส่ง Form ID 'restoreForm' เข้าไป -->
                <button type="button" class="btn btn-soft btn-success" onclick="openConfirmModal('restoreForm')">ยืนยันการกู้คืน</button>
...
<dialog id="Modal_del" class="modal">
    <div class="modal-box">
        <h3 class="font-bold text-lg text-error mb-4">ลบกลุ่ม</h3>
